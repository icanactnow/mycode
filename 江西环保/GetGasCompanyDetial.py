...
class GetGasCompanyDetial(object):
   """docstring for GetGasCompanyDetial"""
...
# 获取单个公司的详细信息，返回dict
   def get_company_detial(self, company_id):
      ...
      # resp = _amf(message, body, self.url)
      if resp.ok:
   ...
   def save_all_company_detial(self):
      # co = GetGasCommpanyNameAndCompanyId()
      ...
      for item in company_id_list:
         # companny_detial_dic = {}
         # companny_detial_dic = self.get_company_detial(item[1])
         # 获取所有工厂信息，保存到list中。
         company_detial_lists.append(self.get_company_detial(item[1]))
         # print(self.get_company_detial(item[1]))
      # 保存到数据库
      self.save_detial(company_detial_lists)
      # return company_detial_lists
if __name__ == '__main__':
   g = GetGasCompanyDetial().save_all_company_detial()
   # g1 = GetGasCompanyDetial().get_company_detial('609230001')
   # print(g)
